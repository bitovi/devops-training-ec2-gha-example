...
if [[ ${#TF_STATE_BUCKET} > 63 ]]; then
  ...
  echo "File only needed to test bucket access. Shouldn't be here. Can be safely deleted" >> statecheck.txt
  if ! [[ -z $(aws s3api put-object --bucket $TF_STATE_BUCKET --key statecheck.txt --body ./statecheck.txt 2>&1 >/dev/null ) ]]; then
    echo "Permission issue: Unable to write to the bucket."
    exit 63
  else
      if ! [[ $(aws s3 rm "s3://$TF_STATE_BUCKET/statecheck.txt" 2>&1 >/dev/null ) ]]; then
